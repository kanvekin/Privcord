...
export async function importSettings(data: string) {
    ...
    if (!("settings" in parsed || "quickCss" in parsed || "dataStore" in parsed)) {
        throw new Error("Invalid Settings. Is this even a Privcord settings file?");
    }
}
...
export async function downloadSettingsBackup(type: BackupType, { minify }: { minify?: boolean; } = {}) {
    ...
    const filename = `privcord-${type}-backup-${moment().format("YYYY-MM-DD")}.json`;
...
export async function uploadSettingsBackup(showToast = true): Promise<void> {
    if (IS_DISCORD_DESKTOP) {
        const [file] = await DiscordNative.fileManager.openFiles({
            filters: [
                { name: "Privcord Settings Backup", extensions: ["json"] },
                { name: "all", extensions: ["*"] }
            ]
        });
